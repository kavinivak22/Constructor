...
export function ExpenseFormSheet({ isOpen, setIsOpen, projectId, expense, onSuccess }: ExpenseFormSheetProps) {
  ...
  useEffect(() => {
    if (expense) {
      form.reset({
        projectId: projectId || expense.project_id || expense.projectId, // snake_case from DB with fallback
        amount: expense.amount,
        category: expense.category,
        description: expense.description,
        expenseDate: new Date((expense.expense_date || expense.expenseDate) as any),
      });
    } else {
      form.reset({
        projectId: projectId || '',
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      const expenseData = {
        ...
        expense_date: values.expenseDate.toISOString().split('T')[0], // date only (YYYY-MM-DD)
      };
