...
export function ChatMessageInput({ projectId }: ChatMessageInputProps) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Add the new message to the chat_messages table
      ...
      if (otherUsers.length > 0) {
        const notifications = otherUsers.map(otherUserId => ({
          ...
          body: `${user.user_metadata?.full_name || user.email}: "${message.substring(0, 50)}${message.length > 50 ? '...' : ''}"`,
          message: `${user.user_metadata?.full_name || user.email}: "${message.substring(0, 50)}${message.length > 50 ? '...' : ''}"`,
          read: false,
        ...
        if (notificationError) {
          // Non-critical — log but don't block message send
          console.warn('Failed to create notifications:', notificationError.message);
        }
      }
