    ...
    const fetchProfile = async () => {
        ...
        try {
            ...
            const { data: profileData, error: profileError } = await supabase
                ...
                .select(`
                    *,
                    companies!users_companyId_fkey (
                        id,
                        name
                    )
                `)
            ...
            if (projectIds.length > 0) {
                const { data: projectsData, error: projectsError } = await supabase
                    .from('projects')
                    .select('id, name, status, client_name, end_date')
                    .in('id', projectIds);
            ...
            setError('Failed to load profile data: ' + (error?.message || error?.details || error?.hint || JSON.stringify(error)));
        } finally {
            setLoading(false);
        }
    };
                                                ...
                                                {projects.map((project) => (
                                                    <div key={project.id} className="flex items-center justify-between p-4 border rounded-lg bg-background hover:bg-muted/50 transition-colors">
                                                        <div className="space-y-1">
                                                            ...
                                                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                                                <span>{project.client_name || project.clientName || 'Internal Project'}</span>
                                                                <span>•</span>
                                                        ...
                                                        {(project.end_date || project.endDate) && (
                                                            <div className="text-xs text-muted-foreground text-right">
                                                                <p>Due</p>
                                                                <p>{format(new Date(project.end_date || project.endDate), 'MMM d, yyyy')}</p>
                                                            </div>
                                                        )}
                                                    </div>
                                                ))}
