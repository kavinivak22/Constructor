...
const formSchema = z.object({
  ...
  siteSelection: z.string().min(1, 'Please select a site or select Add New.'),
  newSiteName: z.string().optional(),
}).refine((data) => {
  if (data.siteSelection === 'new') {
    return !!data.newSiteName && data.newSiteName.trim().length > 0;
  }
  return true;
}, {
  message: 'Please enter a name for the new site.',
  path: ['newSiteName'],
});
...
export default function CreateProjectPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      let siteId = values.siteSelection;

      if (values.siteSelection === 'new') {
      ...
      const { data: projectData, error: projectError } = await supabase.from('projects').insert([
        {
          ...
          client_contact: values.clientContact || null,
          ...
          status: 'active', // Default status matching DB CHECK constraint
          company_id: userProfile.companyId,
          thumbnail_url: imageId || null,
          created_by: user.id,
          site_id: siteId
        },
      ]).select().single();
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <Card className="glass-card">
                ...
                <CardContent className="space-y-4">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Site (Shared Inventory) *</FormLabel>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                          <FormControl>
                            <SelectTrigger>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g., Sector 62 Site" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
              ...
              <Card className="glass-card">
                ...
                <CardContent>
                  <div className="flex items-center justify-center w-full">
                    <label htmlFor="logo-upload" className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-white/10 dark:bg-black/20 hover:bg-white/20 dark:hover:bg-black/30 transition-all relative overflow-hidden">
                      {logoPreviewUrl ? (
              ...
              <Card className="glass-card">
                ...
                <CardContent>
                  <div className="flex items-center justify-center w-full">
                    <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-white/10 dark:bg-black/20 hover:bg-white/20 dark:hover:bg-black/30 transition-all relative overflow-hidden">
                      {previewUrl ? (
