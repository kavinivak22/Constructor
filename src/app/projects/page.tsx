...
export default function ProjectsPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent">
      <header className="flex flex-row items-center justify-between gap-3 p-3.5 sm:p-4 md:px-6 shrink-0 bg-transparent sticky top-0 z-10 glass border-b border-white/10">
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight font-headline text-foreground">
          Projects
        </h1>
        {isAdminOrManager && (
          <Link href="/projects/create">
            <Button size="sm" className="h-9 text-xs sm:text-sm rounded-xl">
      ...
      <main className="flex-1 p-3.5 sm:p-4 md:p-6 overflow-y-auto bg-transparent">
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row gap-2.5 sm:gap-3">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-9 w-full h-9 sm:h-10 text-xs sm:text-sm rounded-xl"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <Select onValueChange={setStatusFilter} value={statusFilter}>
            <SelectTrigger className="w-full sm:w-[180px] h-9 sm:h-10 text-xs sm:text-sm rounded-xl">
        ...
        {isLoading && (
          <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {[...Array(8)].map((_, i) => <Skeleton key={i} className="h-[20rem] rounded-2xl" />)}
          </div>
        )}
        ...
        {!isLoading && filteredAndSortedProjects.length === 0 && (
          <Card className="glass-card flex flex-col items-center justify-center h-64 text-center p-6 rounded-2xl">
            <h3 className="text-lg sm:text-xl font-bold font-headline">No Projects Found</h3>
            <p className="max-w-sm mt-2 text-xs sm:text-sm text-muted-foreground">
