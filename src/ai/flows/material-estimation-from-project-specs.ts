...
/**
 * Streams material estimation text chunk by chunk.
 * @param input - The project details
 * @param onChunk - Callback fired for each streamed text chunk
 * @returns The complete estimation text
 */
export async function getMaterialEstimationStream(
  input: MaterialEstimationInput,
  onChunk: (chunk: string) => void
): Promise<string> {
  const prompt = `You are an expert construction project manager. Based on the project specifications provided, estimate the materials needed for the project.
  ...
  // Await the final response to ensure completion
  await response;
  return fullText;
}
